...
PISTON_GAUGE_HEIGHT = 50 # <--- НОВАТА КОНСТАНТА ЗА ВИСОЧИНА
...
class SolenoidApp:
    def __init__(self, root):
        ...
        self.overtemp_labels, self.solenoid_on_times = [], {}
        ...
        try:
            with open("tester_log.txt", "w", encoding="utf-8") as f: f.write(f"=== TEST SESSION STARTED AT {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n")
    ...
    def create_widget_lists(self, count):
        return { "vars": [tk.StringVar(value="---") for _ in range(count)], "labels": [None] * count, "canvases": [None] * count, "channel_map": {} }
    ...
    def build_sidebar(self, parent):
        ...
        pressure_container = ctk.CTkFrame(self.sidebar_frame, fg_color="transparent")
        pressure_container.pack(side="bottom", fill="x", pady=20, padx=10)
        ctk.CTkLabel(pressure_container, text="System Pressure", font=("Arial", 14, "bold"), text_color="#aaaaaa").pack()
        self.pressure_value_var = tk.StringVar(value="--- bar")
        self.pressure_value_label = ctk.CTkLabel(pressure_container, textvariable=self.pressure_value_var, width=180, height=80, fg_color="#333333", text_color="white", corner_radius=8, font=("Arial", 28, "bold"))
        self.pressure_value_label.pack(pady=(5, 0))
    ...
    def show_page(self, page_name):
        for page in [self.page_manual_solenoids, self.page_auto_tests, self.page_sensors, self.page_logs]: page.pack_forget()
        buttons = {"manual_solenoids": self.btn_manual_solenoids, "auto_tests": self.btn_auto_tests, "sensors": self.btn_sensors, "logs": self.btn_logs}
        ...
        pages = {"manual_solenoids": self.page_manual_solenoids, "auto_tests": self.page_auto_tests, "sensors": self.page_sensors, "logs": self.page_logs}
    ...
    def update_interlock_states(self):
        ...
        for btn_list in [self.solenoid_buttons, self.predefined_buttons, self.individual_piston_buttons]:
            if btn_list:
                for btn in btn_list: btn.configure(state=test_state)
    ...
    def build_info_panel(self, parent):
        info_frame = ctk.CTkFrame(parent, corner_radius=10, fg_color="#18181a", border_width=1, border_color="#333333")
    ...
    def generate_pdf_report(self):
        ...
        info_data = [[Paragraph(f"<b>TCU Block Serial Number:</b> {serial_no}", styles['Normal']), Paragraph("<b>Test System:</b> ZF GS3 Bench V1.0", styles['Normal'])], [Paragraph("<b>Standard / Compatibility:</b> ZF AS-Tronic", styles['Normal']), Paragraph(f"<b>Connection Port:</b> {self.port_var.get() or 'N/A'}", styles['Normal'])]]
    ...
    def build_solenoid_panel(self, parent):
        ...
        headers = ["Channel", "State", "Control", "Temperature"]
        for col, header in enumerate(headers):
            ctk.CTkLabel(solenoid_frame, text=header, font=("Arial", 11, "bold")).grid(row=0, column=col if col < 2 else 2, columnspan=2 if header == "Control" else 1, padx=5, pady=5)
        for i, name in enumerate(SOLENOID_NAMES, start=1):
            ...
            state_label = ctk.CTkLabel(solenoid_frame, textvariable=state_var, width=80, height=26, fg_color="#444444", text_color="white", corner_radius=6, font=("Arial", 10, "bold"))
            ...
            overtemp_label = ctk.CTkLabel(solenoid_frame, text="Standby", width=90, height=26, fg_color=MATTE_GRAY, text_color="white", corner_radius=6, font=("Arial", 9, "bold"))
            overtemp_label.grid(row=i, column=4, padx=10, pady=6)
            self.overtemp_labels.append(overtemp_label)
            
        ttk.Separator(solenoid_frame, orient="horizontal").grid(row=len(SOLENOID_NAMES) + 1, column=0, columnspan=5, sticky="ew", pady=10)
    ...
    def build_sensor_panel(self, parent):
        ...
        for i, (title, channel) in enumerate(SENSOR_DISPLAY_CONFIG):
            ...
            card_frame.grid(row=i, column=0, pady=10, padx=10, sticky="nsew")
            ctk.CTkLabel(card_frame, text=f"{title} (CH {channel})", font=("Arial", 14, "bold")).pack(pady=(8, 4))
            ...
            canvas = tk.Canvas(row_frame, height=50, bg="#2b2b2b", highlightthickness=0)
        ...
        self.check_sensors_btn.grid(row=len(SENSOR_DISPLAY_CONFIG), column=0, pady=(15, 10), padx=10)
    ...
    def draw_sensor_gauge(self, index, value_text):
        ...
        y_start = (h/2) - (bar_h/2)
        canvas.delete("all")
        canvas.create_rectangle(15, y_start, 435, y_start + bar_h, fill="#444444", outline="")
        canvas.create_rectangle(172, y_start, 277, y_start + bar_h, fill=MATTE_GREEN, outline="")
        canvas.create_text(15, y_start + bar_h + 10, text="50", fill="#888888", font=("Arial", 11))
        canvas.create_text(172, y_start + bar_h + 10, text="65", fill="#cccccc", font=("Arial", 11, "bold"))
        canvas.create_text(277, y_start + bar_h + 10, text="75", fill="#cccccc", font=("Arial", 11, "bold"))
        canvas.create_text(435, y_start + bar_h + 10, text="90", fill="#888888", font=("Arial", 11))
        try:
            val = float(value_text.split()[0])
            display_val = max(50, min(90, val))
            x = 15 + ((display_val - 50) / 40.0) * 420
            canvas.create_line(x, y_start - 5, x, y_start + bar_h + 5, fill="white", width=2)
            canvas.create_polygon(x-5, y_start - 5, x+5, y_start - 5, x, y_start, fill="white")
    ...
    def build_simple_inductance_panel(self, parent, widget_vars, title, sticky, col=1, row=0):
        inductance_frame = ctk.CTkFrame(parent, corner_radius=10)
        inductance_frame.grid(row=row, column=col, sticky=sticky, padx=10, pady=10)
        inductance_frame.columnconfigure(0, weight=1)
        ctk.CTkLabel(inductance_frame, text=title, font=("Arial", 11, "bold")).grid(row=0, column=0, pady=5)
    ...
    def draw_piston_gauge(self, index, value_text, widget_vars):
        ...
        canvas.update()
        # КОРЕКЦИЯ: Използваме новата константа
        h, bar_h = canvas.winfo_height(), PISTON_GAUGE_HEIGHT
        y_start = (h/2) - (bar_h/2)
        canvas.delete("all")
        ...
        canvas.create_text(x_offset, y_start + bar_h + 10, text=str(min_val), fill="#888888", font=("Arial", 11))
        canvas.create_text((x_in_start + x_in_end)/2, y_start + bar_h + 10, text=f"IN: {in_start}-{in_end}", fill="#cccccc", font=("Arial", 11, "bold"))
        canvas.create_text((x_out_start + x_out_end)/2, y_start + bar_h + 10, text=f"OUT: {out_start}-{out_end}", fill="#cccccc", font=("Arial", 11, "bold"))
        canvas.create_text(x_offset + w_pixels, y_start + bar_h + 10, text=str(max_val), fill="#888888", font=("Arial", 11))
        try:
            val = float(value_text.split()[0])
    ...
    def set_state(self, channel, state):
        if not (0 <= (index := channel - 1) < len(self.state_vars)): return
        self.current_states[index] = state
        self.state_vars[index].set(state)
        self.state_labels[index].configure(fg_color=MATTE_GREEN if state == "ON" else MATTE_RED if state == "OFF" else "#444444")
        if state == "ON":
            if channel not in self.solenoid_on_times: self.solenoid_on_times[channel] = time.time()
        else:
            if channel in self.solenoid_on_times:
                del self.solenoid_on_times[channel]
                self.overtemp_labels[index].configure(text="Standby", fg_color=MATTE_GRAY)
    ...
    def send_command(self, cmd):
        if not (self.ser and self.ser.is_open):
            ...
            return
        try:
            # КОРЕКЦИЯ: Поправяме двойната наклонена черта от \\n на \n
            self.ser.write((cmd + "\n").encode("utf-8"))
    ...
    def apply_sensor_style(self, widget_index, raw_value):
        label, color = self.sensor_value_labels[widget_index], "#444444"
        self.draw_sensor_gauge(widget_index, raw_value)
        try:
            if 65 <= float(raw_value.split()[0]) <= 75: color = MATTE_GREEN
            else: color = MATTE_RED
        except (ValueError, IndexError): pass
    ...
    def apply_inductance_style(self, widget_vars, widget_index, raw_value):
        ...
        if not label: return
        self.draw_piston_gauge(widget_index, raw_value, widget_vars)
        color = "#444444"
        try:
            val = float(raw_value)
            _, _, _, in_start, in_end, out_start, out_end, _ = INDUCTANCE_DISPLAY_CONFIG[widget_index]
            if (in_start <= val <= in_end) or (out_start <= val <= out_end): color = MATTE_GREEN
            else: color = MATTE_RED
    ...
    def reset_solenoid_states(self):
        for i in range(len(SOLENOID_NAMES)): self.set_state(i + 1, "UNKNOWN")
    ...
    def parse_serial_line(self, line):
        ...
        if not parts: return
        cmd, *args = parts
        handlers = {"SENSORS": self.parse_sensors, "INDUCTANCE": self.parse_inductance_data, "ALL": self.parse_all_states, "PRESSURE": lambda a: self.set_pressure(a[0]) if a else None, "SOL": lambda a: self.set_state(int(a[0]), a[1].upper()) if len(a) == 2 and a[0].isdigit() else None, "STATE": lambda a: self.set_state(int(a[0]), a[1].upper()) if len(a) == 2 and a[0].isdigit() else None, "MODE": lambda a: self.mode_var.set(f"Mode: {' '.join(a)}"), "AUTO_STATE": lambda a: self.auto_state_var.set(f"Auto sequence: {' '.join(a).upper()}"), "AUTO_DONE": lambda a: self.complete_auto_sequence(), "AUTO_STEP": lambda a: self.update_auto_step(int(a[0]), int(a[1]), a[2]) if len(a) == 3 and a[0].isdigit() and a[1].isdigit() else None}
